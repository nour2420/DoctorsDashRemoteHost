...
<body class="min-h-screen text-slate-100" style="background: radial-gradient(1200px 600px at 18% 0%, rgba(255,255,255,0.06), transparent 60%), radial-gradient(900px 500px at 85% 20%, rgba(255,255,255,0.04), transparent 65%), var(--bh-bg);">
    ...
    <div class="min-h-screen flex">
        ...
        <aside id="bh-user-sidebar" class="fixed inset-y-0 left-0 z-50 w-72 -translate-x-full md:translate-x-0 md:static md:z-auto md:flex md:w-68 flex-col border-r border-slate-800/80 bg-slate-950/95 transition-transform duration-200" style="background-color: var(--bh-surface-2);">
            <div class="h-16 flex items-center px-6 border-b border-slate-800/80 from-slate-950 via-slate-900 to-slate-950">
                <div class="flex items-center gap-3 w-full">
                    ...
                    <button id="bh-user-sidebar-close" type="button" class="ml-auto inline-flex md:hidden items-center justify-center rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-white/90 hover:bg-white/10">
            ...
            <nav class="flex-1 px-4 py-4 space-y-1 text-base md:text-lg">
                <a href="{{ route('user.dashboard') }}"
                    class="bh-nav-link block rounded-lg px-3 py-2 pl-7 text-sm md:text-base font-medium {{ request()->routeIs('user.dashboard') ? 'bh-nav-link-active' : 'text-slate-300 hover:bg-slate-900 hover:text-white border border-transparent' }}">Dashboard</a>
                <a href="{{ route('user.reports.index') }}"
                    class="bh-nav-link block rounded-lg px-3 py-2 pl-7 text-sm md:text-base font-medium {{ request()->routeIs('user.reports.*') ? 'bh-nav-link-active' : 'text-slate-300 hover:bg-slate-900 hover:text-white border border-transparent' }}">My Cases</a>
                <div class="pt-4 mt-4 border-t border-slate-800 text-[13px] font-semibold text-slate-400 uppercase tracking-[0.18em]">
                    Chats</div>
                <a href="{{ route('user.chats.doctors') }}"
                    class="bh-nav-link block rounded-lg px-3 py-2 pl-7 text-sm md:text-base font-medium {{ request()->routeIs('user.chats.doctors*') ? 'bh-nav-link-active' : 'text-slate-300 hover:bg-slate-900 hover:text-white border border-transparent' }}">With Admin</a>
                <a href="{{ route('user.chats.group') }}"
                    class="bh-nav-link block rounded-lg px-3 py-2 pl-7 text-sm md:text-base font-medium {{ request()->routeIs('user.chats.group*') ? 'bh-nav-link-active' : 'text-slate-300 hover:bg-slate-900 hover:text-white border border-transparent' }}">With Admins Group</a>
            </nav>
        </aside>
        ...
        <div class="flex-1 flex flex-col">
            <header class="h-16 border-b border-slate-800/80 flex items-center justify-between px-4 md:px-8 bg-black/70 backdrop-blur-xl" style="background-color: rgba(12,12,12,0.78);">
                <div class="flex items-center gap-2 md:hidden">
                    <button id="bh-user-sidebar-toggle" type="button" class="inline-flex items-center justify-center rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-white/90 hover:bg-white/10">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-5 w-5">
                            <path d="M4 6h16" />
                            <path d="M4 12h16" />
                            <path d="M4 18h16" />
    ...
    <script>
        (function () {
            function initUserSidebar() {
                var sidebar = document.getElementById('bh-user-sidebar');
                var overlay = document.getElementById('bh-user-sidebar-overlay');
                var toggleBtn = document.getElementById('bh-user-sidebar-toggle');
                var closeBtn = document.getElementById('bh-user-sidebar-close');

                if (!sidebar || !overlay || !toggleBtn || !closeBtn) return;
                if (toggleBtn.dataset.bhBound === '1') return;

                toggleBtn.dataset.bhBound = '1';
    ...
    <script>
        (function () {
            ...
            document.addEventListener('submit', function (e) {
                ...
                // Show global loading on form submit
                showGlobalLoading();
            }, true);
